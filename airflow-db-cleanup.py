...
cleanup_BaseJob_op = PythonOperator(
    ...
    params=DATABASE_OBJECTS[0],
    provide_context=True,
    dag=dag
)

cleanup_Log_op = PythonOperator(
    task_id='cleanup_Log_op',
    python_callable=cleanup_function,
    params=DATABASE_OBJECTS[1],
    provide_context=True,
    dag=dag
)

cleanup_DagModel_op = PythonOperator(
    task_id='cleanup_DagModel_op',
    python_callable=cleanup_function,
    params=DATABASE_OBJECTS[2],
    provide_context=True,
    dag=dag
)

cleanup_TaskFail_op = PythonOperator(
    task_id='cleanup_TaskFail_op',
    python_callable=cleanup_function,
    params=DATABASE_OBJECTS[3],
    provide_context=True,
    dag=dag
)
...
cleanup_DagRun_op = PythonOperator(
    ...
    python_callable=cleanup_function,
    params=DATABASE_OBJECTS[5],
    provide_context=True,
    dag=dag
)

cleanup_RaskReschedule_op = PythonOperator(
    task_id='cleanup_RaskReschedule_op',
    python_callable=cleanup_function,
    params=DATABASE_OBJECTS[6],
    provide_context=True,
    dag=dag
)

cleanup_RenderTaskInstanceFields_op = PythonOperator(
    task_id='cleanup_RenderTaskInstanceFields_op',
    python_callable=cleanup_function,
    params=DATABASE_OBJECTS[7],
    provide_context=True,
    dag=dag
)

cleanup_ImportError_op = PythonOperator(
    task_id='cleanup_ImportError_op',
    python_callable=cleanup_function,
    params=DATABASE_OBJECTS[8],
    provide_context=True,
    dag=dag
)

cleanup_BaseXCom_op = PythonOperator(
    task_id='cleanup_' + str(DATABASE_OBJECTS[9]["airflow_db_model"].__name__),
    python_callable=cleanup_function,
    params=DATABASE_OBJECTS[9],
    provide_context=True,
    dag=dag
)

cleanup_SlaMiss_op = PythonOperator(
    task_id='cleanup_SlaMiss_op',
    python_callable=cleanup_function,
    params=DATABASE_OBJECTS[10],
    provide_context=True,
    dag=dag
)

cleanup_TaskSet_op = PythonOperator(
    task_id='cleanup_TaskSet_op',
    python_callable=cleanup_function,
    params=DATABASE_OBJECTS[11],
    provide_context=True,
    dag=dag
)

cleanup_TaskInstace_op = PythonOperator(
    task_id='cleanup_TaskInstace_op',
    python_callable=cleanup_function,
    params=DATABASE_OBJECTS[12],
    provide_context=True,
    dag=dag
)
